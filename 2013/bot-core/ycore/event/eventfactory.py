...
class MessageEventFactory(EventFactory):
    ...
    def makeEvent(self):
        ...
        if (typeM == Messages.MSG_TYPE_PRIV):
            ...
            args = f[0][4].split(" ")
            args.pop(0)
            event = evt.MessagePrivateCommandEvent(f[0][0], f[0][3].lower(), args)
        elif (typeM == Messages.MSG_TYPE_ARENA):
            event = evt.MessageArenaEvent(self.message)
        elif (typeM == Messages.MSG_TYPE_MONEY_GIVE):
            f = re.findall(Messages.MSG_ARENA_MONEY, self.message)
            event = evt.MessageArenaMoneyEvent(f[0][0], int(f[0][1]), f[0][3])
        return event
...
class Messages:
    ...
    MSG_ARENA_MONEY = "^MSG:ARENA:Player (.+?) gave you \\$(.+?)\\.( Message: \"(.+?)\")?$"
    MSG_TYPE_UNKNOWN, MSG_TYPE_PRIV, MSG_TYPE_CMD, MSG_TYPE_ARENA, MSG_TYPE_MONEY_GIVE = range(5)

    @staticmethod
    def getMessageType(message):
        typeM = Messages.MSG_TYPE_UNKNOWN
        if (message.startswith(Messages.MSG_PRIV)):
            if (re.match(Messages.MSG_PRIV_CMD, message)):
                typeM = Messages.MSG_TYPE_CMD
            else:
                typeM = Messages.MSG_TYPE_PRIV
        elif (message.startswith(Messages.MSG_ARENA)):
            if (re.match(Messages.MSG_ARENA_MONEY, message)):
                typeM = Messages.MSG_TYPE_MONEY_GIVE
            else:
